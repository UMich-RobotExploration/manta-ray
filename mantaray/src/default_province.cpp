...
int main() {
  ...
  FlatBoundary3D(bhContext.params().bdinfo->top, 0.0, {-10.0, 10.0},
                 {-10.0, 10.0});
  bhc::extsetup_bathymetry(bhContext.params(), {11, 21}, 5);
  FlatBoundary3D(bhContext.params().bdinfo->bot, 100.0,
  ...
  for (int ix = 0; ix < 11; ++ix) {
    for (int iy = 0; iy < 21; ++iy) {
      bhContext.params().bdinfo->bot.bd[ix * 21 + iy].Province =
          GetProvince(ix, iy);
    }
  }
  int cnt = 0;
